...
get_stats <- function(results_file = ODE_results,
 ...
 # finding the time system reaches stable equilibrium  and fill in the raw results
  ...
  for(i in 1:length(to_test)){
    ...
    print(paste("searching equilbrium for combination", i))
    if(all(is.na(eq_raw))){
  ...
  if(is.na(test_parameters) == TRUE){
    ...
    eq_met <- pivot_longer(eq_dat, all_of(test_parameters), names_to = "parameter")
  }

  eq_met <- pivot_longer(eq_met,eq_infect,
                         names_to = "infection_status", values_to = "proportion")
  ...
  ld <- function(coinf, N0, A, B, A_plus = 0, B_plus = 0){
...
print(paste("Calculating correlations at equilibrium"))
  ...
  if(length(cor_param_method) >= 1){
    if(any(cor_param_method == "ld")){
      ...
      eq_param_cor <- eq_av_prop %>% group_by(parameter, infection_status) %>% summarise(cor = cor(value, proportion, method = corT))
    ...
    if(any(cor_param_method == "ld")){
      Stats <- list(model_det = model_det,
                    "equilibrum_stats" = list("Results" = eq_raw,
